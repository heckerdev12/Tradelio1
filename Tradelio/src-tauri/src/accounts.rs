...
pub fn add_account(
    ...
    conn.execute(
        "INSERT INTO accounts (name, broker, account_number, account_nickname, account_type, account_plan, balance, leverage, trading_terminal, created_at)
...
pub fn get_all_accounts(app: AppHandle) -> Result<Vec<TradingAccount>, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, name, broker, account_number, account_nickname, account_type, account_plan, balance, leverage, trading_terminal, created_at FROM accounts ORDER BY created_at DESC")
...
pub fn update_account_balance(
    ...
    println!("Updating account {} balance to {}", account_id, new_balance);
...
pub fn get_all_transactions(app: AppHandle) -> Result<Vec<Transaction>, String> {
    ...
    let mut stmt = conn
        .prepare("SELECT id, account_name, transaction_type, amount, date, created_at FROM transactions ORDER BY date DESC")
        .map_err(|e| e.to_string())?;
...
pub fn get_transactions_by_account(
    ...
    let mut stmt = conn
        .prepare("SELECT id, account_name, transaction_type, amount, date, created_at FROM transactions WHERE account_name = ?1 ORDER BY date DESC")
        .map_err(|e| e.to_string())?;
    ...
    conn.execute(
        "UPDATE accounts
         SET name = ?1, broker = ?2, account_number = ?3, account_nickname = ?4,
             account_type = ?5, account_plan = ?6, balance = ?7, leverage = ?8,
             trading_terminal = ?9, created_at = ?10
         WHERE id = ?11",
        params![
            account.name,
            account.broker,
